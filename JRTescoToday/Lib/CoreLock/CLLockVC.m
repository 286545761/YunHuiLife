...
@interface CLLockVC ()
{
      NSUserDefaults *myUserDefaults;//判断手势密码状态
}
/** 操作成功：密码设置成功、密码验证成功 */
...
- (void)viewDidLoad {
    ...
    //数据传输
    [self dataTransfer];
   //  [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(downLoadFinish11) name:@"SET_HAND_PWD1" object:nil];
    //事件
    [self event];
}
...
{
    NSString *str=@"Error";
    [FNUserDefaults setObject:str forKey:@"headPassword"];
...
-(void)dismiss{
    [_actionView removeFromSuperview];
    NSString *str=@"Error";
    [myUserDefaults setObject:str forKey:@"headPassword"];
       [[NSNotificationCenter defaultCenter] postNotificationName:LOGIN_SUCCESS object:nil];
    [self dismiss:0];
}
 ...
 */
-(void)forgetPwd{
    LoginController *str=[[LoginController alloc] init];
...
+(instancetype)lockVC:(UIViewController *)vc{
    ...
    //[vc.navigationController  pushViewController:navVC animated:YES];
...
- (IBAction)forgetPwdAction:(id)sender {
    ...
    LoginController *str=[[LoginController alloc] init];
  UINavigationController *avc=[[UINavigationController alloc] initWithRootViewController:str];
