...
{
    NSString *normalDateStr = [NSString string];
    NSDate *date = [NSDate date];
    NSDateFormatter *formatter = [[NSDateFormatter alloc] init];
    [formatter setDateFormat:@"HHmmssss"];
    normalDateStr = [formatter stringFromDate:date];
...
+ (NSString *)getAllKeysWithSort:(NSDictionary *)dic
{
    ...
    NSMutableString *b = [NSMutableString string]; // 为字典拼接间隔符
    ...
    for (int i = 0; i < sortKeysArray.count; i++) {
        ...
        b = [NSMutableString stringWithFormat:@"%@=%@",sortKeysArray[i],[dic objectForKey:sortKeysArray[i]]];
...
+ (NSString *)getPulicKey
{
    ...
    NSString *publicKey = [defaults objectForKey:@"publicKey"];
    
    if (!publicKey) [self loadPublicKey];
...
+ (void)loadPublicKey
{
    ...
    [HttpTool postWithBaseURL:kBaseURL path:@"public/key/query" params:nil success:^(id data) {
        
        NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
        
        NSString *pk = [defaults objectForKey:@"publicKey"];
        
        if (!pk | ![pk isEqualToString:data[@"publicKey"]]) {
            
            [defaults setObject:data[@"publicKey"] forKey:@"publicKey"];
