...
#import "MJRefresh.h"
@interface JROnSchelvesViewController ()<UITableViewDelegate, UITableViewDataSource,JRProductManageCellDelegate>
...
@implementation JROnSchelvesViewController
...
- (void)viewDidLoad {
    ...
    self.tableView = [[UITableView alloc] initWithFrame:CGRectMake(0,0, [UIScreen mainScreen].bounds.size.width, [UIScreen mainScreen].bounds.size.height - 65-49-44) style:UITableViewStylePlain];
...
- (void)add_MJRefresh {
    ...
    self.tableView.header = freshHeader;
...
- (void)loadData {
    ...
    [JRBusinessCenterCore query_requestBusinessManagerWithToken:token type:@"2" pageNum:[NSString stringWithFormat:@"%ld",self.page] pageSize:nil Successed:^(NSDictionary *data) {
