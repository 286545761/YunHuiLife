...
- (void)setupCommonUI
{
    ...
    /*! 添加手势 */
    if (self.blurTapStyle == DSAlertTapStyleALLScreen) {
...
- (UIImageView *)blurImageView
{
    if ( !_blurImageView )
    {
        ...
        /*! 添加手势 */
        if (_blurTapStyle == DSAlertTapStyleALlSides) {
            [_blurImageView addGestureRecognizer:self.dismissTap];
...
- (void)setIsTouchEdgeHide:(BOOL)isTouchEdgeHide
{
    ...
    _blurTapStyle = DSAlertTapStyleALlSides;
}
...
- (void)addLabel:(UILabel *)label maxHeight:(CGFloat)maxHeight
{
    ...
    label.frame   = CGRectMake(kDSAlertPaddingH, _scrollBottom+10, _maxContentWidth, size.height);
